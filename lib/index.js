...
var YoutubeClient = (function () {
  ...
  YoutubeClient.prototype.getChannels = function (ids) {
    ...
    return this.httpRequest.get(url).then(function (res) { return fromYoutubeChannels(res); });
  };
  ...
  YoutubeClient.prototype.getPlaylists = function (ids) {
    ...
    return this.httpRequest.get(url).then(function (res) {
      var r = fromYoutubePlaylists(res);
      return r.list;
    });
  };
  ...
  YoutubeClient.prototype.getVideo = function (id, noSnippet) {
    return this.getVideos([id], noSnippet).then(function (res) { return res && res.length > 0 ? res[0] : null; });
  };
  YoutubeClient.prototype.getCommentThreads = function (videoId, sort, max, nextPageToken) {
    ...
    var pageToken = (nextPageToken ? "&pageToken=" + nextPageToken : '');
    var url = "https://www.googleapis.com/youtube/v3/commentThreads?key=" + this.key + "&videoId=" + videoId + orderParam + "&maxResults=" + maxResults + pageToken + "&part=snippet";
    return this.httpRequest.get(url).then(function (res) { return fromYoutubeCommentThreads(res); });
  };
  YoutubeClient.prototype.getComments = function (id, max, nextPageToken) {
    var maxResults = (max && max > 0 ? max : 20);
    var pageToken = (nextPageToken ? "&pageToken=" + nextPageToken : '');
    var url = "https://www.googleapis.com/youtube/v3/comments?key=" + this.key + "&parentId=" + id + "&maxResults=" + maxResults + pageToken + "&part=snippet";
    return this.httpRequest.get(url).then(function (res) { return fromYoutubeComments(res); });
  };
  YoutubeClient.prototype.search = function (sm, max, nextPageToken) {
    var searchType = sm.type ? "&type=" + sm.type : '';
    var searchDuration = sm.type === 'video' && (sm.videoDuration === 'long' || sm.videoDuration === 'medium' || sm.videoDuration === 'short') ? "&videoDuration=" + sm.videoDuration : '';
    var searchOrder = (sm.order === 'date' || sm.order === 'rating' || sm.order === 'title' || sm.order === 'videoCount' || sm.order === 'viewCount') ? "&order=" + sm.order : '';
    var regionParam = (sm.regionCode && sm.regionCode.length > 0 ? "&regionCode=" + sm.regionCode : '');
    var pageToken = (nextPageToken ? "&pageToken=" + nextPageToken : '');
    var maxResults = (max && max > 0 ? max : 50);
    var url = "https://www.googleapis.com/youtube/v3/search?key=" + this.key + "&part=snippet" + regionParam + "&q=" + sm.keyword + "&maxResults=" + maxResults + searchType + searchDuration + searchOrder + pageToken;
    return this.httpRequest.get(url).then(function (res) { return fromYoutubeSearch(res); });
  };
  ...
  YoutubeClient.prototype.getRelatedVideos = function (videoId, max, nextPageToken) {
    var maxResults = (max && max > 0 ? max : 12);
    var pageToken = (nextPageToken ? "&pageToken=" + nextPageToken : '');
    var url = "https://youtube.googleapis.com/youtube/v3/search?key=" + this.key + "&relatedToVideoId=" + videoId + "&type=video&maxResults=" + maxResults + pageToken + "&part=snippet";
    return this.httpRequest.get(url).then(function (res) { return fromYoutubeSearch(res); });
  };
...
var DefaultSyncService = (function () {
  function DefaultSyncService(client, repo) {
    this.client = client;
    this.repo = repo;
...
function syncChannel(channelId, client, repo) {
...
function checkAndSyncUploads(channel, channelSync, client, repo) {
  if (!channel.uploads || channel.uploads.length === 0) {
    ...
    var date_1 = new Date();
    var timestamp = channelSync ? channelSync.timestamp : undefined;
    ...
    syncUploads(channel.uploads, client, repo, timestamp).then(function (r) {
      channel.timestamp = r.timestamp;
      channel.count = r.count;
      channel.itemCount = r.all;
      syncChannelPlaylists(channel.id, syncVideos_1, syncCollection_1, client, repo).then(function (res) {
        if (syncCollection_1) {
        ...
        return repo.saveChannel(channel).then(function (c) {
          return repo.saveChannelSync({ id: channel.id, timestamp: date_1, uploads: channel.uploads });
        });
      });
    });
  }
...
function syncPlaylist(playlistId, syncVideos, client, repo) {
  return __awaiter(this, void 0, void 0, function () {
    var res, playlist;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0: return [4, syncPlaylistVideos(playlistId, syncVideos, client, repo)];
        case 1:
          res = _a.sent();
