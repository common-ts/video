...
var YoutubeClient = (function () {
  ...
  YoutubeClient.prototype.getChannels = function (ids) {
    ...
    return this.httpRequest.get(url).then(function (res) { return youtube_1.formatThumbnail(youtube_1.fromYoutubeChannels(res)); });
  };
  ...
  YoutubeClient.prototype.getPlaylistVideos = function (playlistId, max, nextPageToken) {
    ...
    return this.httpRequest.get(url).then(function (res) {
      ...
      r.list = youtube_1.formatThumbnail(r.list);
      return r;
    });
  };
  ...
  YoutubeClient.prototype.search = function (sm, max, nextPageToken) {
    ...
    return this.httpRequest.get(url).then(function (res) { return common_client_1.fromYoutubeSearch(res); });
  };
  ...
  YoutubeClient.prototype.getRelatedVideos = function (videoId, max, nextPageToken) {
    var maxResults = (max && max > 0 ? max : 24);
    var pageToken = (nextPageToken ? "&pageToken=" + nextPageToken : '');
    var url = "https://youtube.googleapis.com/youtube/v3/search?key=" + this.key + "&relatedToVideoId=" + videoId + "&type=video&regionCode=VN&maxResults=" + maxResults + pageToken + "&part=snippet";
    return this.httpRequest.get(url).then(function (res) { return common_client_1.fromYoutubeSearch(res); });
  };
